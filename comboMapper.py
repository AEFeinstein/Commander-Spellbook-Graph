...
def recursiveSearch(comboGraph, subGraph, key, rLevel, minWeight):
    ...
    if (key in comboGraph.keys()):
        ...
        for rKey in list(subGraph[key].keys()):
            if subGraph[key][rKey] < minWeight:
                subGraph[key].pop(rKey)
...
if __name__ == "__main__":
    ...
    with open('values_batchGet.json') as f:
        data = json.load(f)
    ...
    #     data = json.loads(url.read().decode())
        ...
        for combo in values:
            ...
            # Only graph three card combos
            # cardsInCombo = 0
            # for cardIdx in range(10):
            #     if combo[cardIdx]:
            #         cardsInCombo = cardsInCombo + 1
            # if cardsInCombo > 3:
            #     continue
    ...
    queenGraph = dict({})
    recursiveSearch(comboGraph, queenGraph, "Sliver Queen", 2, 2)
